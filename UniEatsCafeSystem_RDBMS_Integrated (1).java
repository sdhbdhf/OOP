    ...
    private void showItemDetails(FoodItem item) {
        ...
        addButton.addActionListener(e -> {
            ...
            if (!newName.isEmpty() && !newDesc.isEmpty() && !newImagePath.isEmpty()) {
                ...
                DBHelper.insertOrUpdateFoodItem(newName, newDesc, newPrice, newImagePath);
 && !newDesc.isEmpty() && !newImagePath.isEmpty()) {
...
class DBHelper {
    private static final String DB_URL = "jdbc:mysql://localhost:3306/UnieatsDB";
    ...
    public static void insertOrUpdateFoodItem(String name, String desc, double price, String imgPath) {
        String sql = "REPLACE INTO menu_items (id, name, description, price, image_path, created_at) " +
                     "VALUES ((SELECT id FROM menu_items WHERE name = ?), ?, ?, ?, ?, NOW())";
